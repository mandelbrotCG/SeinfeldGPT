...
class Plotter:
    ...
    def sample(self, newVals):
        if len(newVals) != len(self.values):
            print("Error in plotter sample: Wrong number of values added")
        else:
            self.curEpoch += self.sampleInterval
            for i in range(len(self.values)):
                self.values[i].append(newVals[i])
    def sampleAndPlot(self, newVals):
        self.sample(newVals)
        self.plot()
